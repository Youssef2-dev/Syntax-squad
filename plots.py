...
# Q3 : Does following the AI-Recommended Path actually lead to higher Quiz Scores?
def create_fig3(filtered_df):
    ...
    ax3.set_title('Impact of Following AI-Recommended Path on Quiz Scores')
    ax3.set_xlabel('Followed AI Recommendation?')
    ax3.set_ylabel('Final Quiz Score')
    return fig3
# /Q3

# Q4 : Which Type of students that uses AI personalized paths and recommendations get affected the most,
...
def create_fig4(filtered_df):
    ...
    ax4.set_title('AI Path Impact: Performance Lift by Student Type', fontsize=14)
    ax4.set_ylabel('Score / Percentage (%)')
    ax4.set_xlabel('Student Performance Category')
    ax4.legend(title='Stage', labels=['Previous GPA (Scaled)', 'Current GPA'])
    return fig4
# /Q4
...
# Q7 : Does time spend per module affects the final score? (more time better score?)
def create_fig7(filtered_df):
    ...
    ax7.set_title('Relationship Between Time Spent and Final Score')
    ax7.set_xlabel('Time Spent Per Module')
    ax7.set_ylabel('Final Score')
    return fig7
# /Q7

# Q8 : Compare previous average grades (GPA) with final (GPA) in the dataset.
def create_fig8(filtered_df):
    ...
    grid8.fig.suptitle('Relationship: Past GPA vs. Current GPA', y=1.02)
    return grid8.fig
# /Q8
